...
class PAPRAS_MuJoCo:
    def __init__(self):
        ...
        self.eef_z_tf_pub = rospy.Publisher("/eef_z_tf", Float64MultiArray, queue_size=100)

        self.publish_period = 0.070
    ...
    def calculate_single_iteration(self):
        if self.new_input != False:
            ...
            eef_z_val = Float64MultiArray()
            eef_z_val.data = eef_frame_point
            self.eef_z_tf_pub.publish(eef_z_val)

            base_frame_t = np.linalg.inv(base_frame_r)
            # print(base_frame_r)
            # print(base_frame_point)
            # eef_frame_t = self.create_transformation_matrix(eef_frame_r,eef_frame_point)
            transformation_matrix = base_frame_t @ eef_frame_r 
...
if __name__ == '__main__':
    ...
    papras_mujoco.q = papras_mujoco.env.get_q_pos(q_pos_idxs=papras_mujoco.env.rev_joint_idxs)
...
#         n_contacts = self.env.sim.data.ncon
#         if n_contacts > 1:
#             new_joint_state.position = curr_pos[1:]
#             print("Collision Detected. Halting robot.")
#             # Visualize collisions
#             for c_idx in range(n_contacts):
#                 contact    = self.env.sim.data.contact[c_idx]
...
#         # for i in range(10):
#         #     point.time_from_start = rospy.Duration(self.publish_period * i) 
#         #     traj_msg.points.append(point)
...
#     rate = rospy.Rate(1.0 / papras_mujoco.publish_period)
#     while not rospy.is_shutdown():
#         papras_mujoco.calculate_single_iteration()
#         papras_mujoco.servo_calc()
#         # papras_mujoco.render()
